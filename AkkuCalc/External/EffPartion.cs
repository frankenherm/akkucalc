 ...
 * 
 * Conditional copmiliation macro:
 * TEST forces to print test and timing messages, it's defined in project Properties, tab Build
 * DBG  forces to print debug messages, it's defined in this file below
 */
...
namespace effPartition
{
    using numb_id = System.UInt32;
    using numb_val = System.Single;
    using ss_id = System.UInt16;
    using ss_sum = System.Single;
    ...
    sealed class effPartition
    {
...
#if TEST
            /// <summary>Prints numbers</summary>
            internal void Print()
            {
                foreach (IdNumber n in this) Console.Write("{0:d} ", n.Val);
                Console.WriteLine();
            }
#endif
        }
        ...
        internal class Partition
        {
            ...
            /// <summary>Method index by call order</summary>
            uint bottomCnt;
            /// <summary>counter of 'up' iterations: used in DSTree()</summary>
            uint upCnt;
#endif
		    ...
		    void DSTree(int currItInd, ss_id invInd)
            {
                ...
                {				// last bin
...
#if DBG
				        Console.Write(">>new  {0:d}\t", currResult.SumDiff);	PrintRange(false);
				        if(complete!=0) Console.Write("\tcomplete: {0:d}", complete);
				        Console.WriteLine();
#endif
                    }
                    ...
                    {
#if DBG
                        Console.WriteLine(">>save {0:d}\t{1:d}", currResult.SumDiff, standbySumDiff);
#endif
                        standbyNumbs.CopyIndexes(numbs);	// keep current numbers as the standby one
            ...
            void ISTree(ss_id ssCnt)
            {
                ...
                do
                {
...
#endif
#if DBG
			        Console.Write("up = {0:d}\tinacc = {1:d}\t", up, sumDiff);	PrintRange(true);
#endif
                    ...
                    {
#if DBG
                        Console.WriteLine(">>saved {0:d}\t{1:d}", currResult.SumDiff, standbySumDiff);
#endif
                        break;
                    }
                }
...
#if !TEST
                if (currResult.SumDiff < finalResult.SumDiff)
#endif
                {
...
#if DBG
	        void PrintRange(bool eol)
	        {
		        Console.Write("[{0:d}-{1:d}]", minSum, maxSum);
...
#if TEST
            /// <summary>Partition method titles to print while testing</summary>
            static string[] methodTitles = new string[] { "UGreedy", "Greedy", "SGreedy", "DSTree" };
#endif
...
#if TEST
                stopwatch.Stop();
                Console.Write("{0:G} ms\t", stopwatch.Elapsed.TotalMilliseconds);
                Console.Write("{0:d}\t{1:f}%", finalResult.SumDiff, 100F * finalResult.SumDiff / avrSum);
                if (i == 2)			// SGreedy
                { if (callCnt > 1)	Console.Write("\titers: {0:d}", callCnt); }
                else if (i == 3)		// DSTree
                    Console.Write("  calls|bottoms|ups: {0:d} {1:d} {2:d}", callCnt, bottomCnt, upCnt);
                Console.WriteLine();
                //finalResult.Print();
                return false;
#else
...
#if TEST
                //numbs.Print();
                Console.WriteLine("avr: {0:f}\t{1:d}...{2:d}", avrSum, numbs[numbs.Count - 1].Val, numbs[0].Val);
#else
